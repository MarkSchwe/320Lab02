...
class BinarySearch{
public:
    int Bsearch(int arr[],int start, int end, int search, int &fails);

};

int BinarySearch::Bsearch(int arr[], int start, int end, int search, int &fails){
    //cout << search << endl;
...
//cout << "TestSmall" << endl;
fails++;
return Bsearch(arr,start,middle,search, fails);
}
//cout << "TestBig" << endl;
if(search > arr[middle] && start != end){
fails++;
return Bsearch(arr,middle,end,search, fails);
}
return 0; 
}
